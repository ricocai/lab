...
(function(window, document) {
    ...
    NETDATA.IAMServer = ''
    NETDATA.OpenAPIServer = ''
    if(typeof IAMServer !== 'undefined'){
      NETDATA.IAMServer = IAMServer;
      console.log('NETDATA.IAMServer = ', NETDATA.IAMServer)
    }
    if(typeof OpenAPIServer !== 'undefined'){
      NETDATA.OpenAPIServer = OpenAPIServer;
      console.log('NETDATA.OpenAPIServer = ', NETDATA.OpenAPIServer)
    }
    if(typeof netdataServer !== 'undefined')
        NETDATA.serverDefault = netdataServer;
    else {
        var s = NETDATA._scriptSource();
        if(s) NETDATA.serverDefault = s.replace(/\/dash_yunkon.js(\?.*)*$/g, "");
        else {
    ...
    //NETDATA.serverDefault = 'http://47.93.79.144:8529/_db/api/v1/'
    console.log('Rico trace in NETDATA.serverDefault = ', NETDATA.serverDefault)
    NETDATA.instance = axios.create({
      baseURL: 'http://47.93.79.144:9110/api/v1/',
      timeout: 1000,
      headers: {
        'X-Access-Token': 'a311ecb67b4800384533f59e64653761'
      }
    });
    NETDATA.IAM = axios.create({
      baseURL: 'http://47.93.79.144:8529/_db/api/v1/',
      timeout: 1000,
      headers: {
        'X-Access-Token': 'a311ecb67b4800384533f59e64653761'
      }
    ...
    //NETDATA.axios               = 'https://unpkg.com/axios/dist/axios.min.js'
...
*/
    ...
    NETDATA.fixHost = function(host) {
        while(host.slice(-1) === '/')
            ...
            console.info('old host:', host);
            // http://localhost:8529/_db/api/v1/api/v1/chart?chart=netdata.server_cpu&_=1500781574657
            host = 'http://47.93.79.144:8529/_db/api/v1/'; // by rico
        return host;
    };
    ...
    NETDATA.chartRegistry = {
        ...
        add: function(host, id, data) {
            ...
            console.log('added ' + host + '/' + id + ' with data:' + data);
        ...
        downloadAll: function(host, callback) {
            host = this.host; //NETDATA.fixHost(host);

            var self = this;
            console.log('Rico trace in downloadAll:',  host + '/api/v1/charts');
    ...
    var chartState = function(element) {
        ...
        console.log('Rico trace in chartState:',self.data('host'))
        console.log('Rico trace in chartState:',NETDATA.chartDefaults.host)
        ...
        console.log('Rico trace in chartState: this.host=', this.host)
        ...
        if(typeof NETDATA.chartLibraries[that.library_name] === 'undefined') {
            ...
            console.log('^^^^^^^^^^^^NETDATA.chartLibraries[] with ', that.library_name)
          }
        ...
        this.chartURL = function() {
            ...
            console.log('this.chart.data_url = ', this.chart.data_url)
            console.dir(this.chart)
            this.data_url = NETDATA.IAMServer + this.chart.data_url;
        ...
        this.updateChart = function(callback) {
            ...
            this._updating = true;
            console.log('>>>>>>>>>>>  Rico trace in updateChart with url=', this.data_url)

            NETDATA.IAM.get(this.data_url)
              //'/api/v1/data?chart=netdata.server_cpu&format=json&points=60&group=average&options=absolute|jsonwrap|nonzero&after=-60&dimensions=user&_=1500779913718')
              .then(function (response) {
                let chart = response.data
                $("#iam").text(JSON.stringify(chart));
                console.log(chart);
                that.xhr = undefined;
                that.retries_on_data_failures = 0;
                that.updateChartWithData(chart);
        ...
        this.autoRefresh = function(callback) {
            if(this.canBeAutoRefreshed() === true && this.running === false) {
                ...
                state.updateChart(function() {
                  console.log('autoRefresh with updateChart')
                    state.running = false;
              ...
              console.log('autoRefresh with undefined')
                if(typeof callback !== 'undefined')
                    return callback();
            }
        };
        ...
        this.getChart = function(callback) {
          console.log('Rico trace in getChart:', this.host)
            ...
            if(this.chart) {
                ...
                console.log(' <<<<<<<<<<< Rico trace in getChart:', this.host + this.chart_url)

                // NETDATA.instance.get('/hostgroups?detail=true', {
                //     params: {
                //       ID: 12345
                //     }
                //   })
                ...
                NETDATA.IAM.get(this.chart_url)
                  .then(function (res) {
                    var chart = res.data
                    $("#iam").text(JSON.stringify(chart));
                    console.log(chart);
                    ...
                    console.log('========== come into always to callback with ok===========')
                      if(typeof callback === 'function')
                          return callback();
                  })
                  .catch(function (error) {
                    ...
                    console.log('========== come into always to callback with error===========')
                      if(typeof callback === 'function')
                          return callback();
                  })
                ...
                // })
                // .always(function() {
                //   console.log('========== come into always to callback ===========')
                //     if(typeof callback === 'function')
                //         return callback();
                // });
            }
    ...
    NETDATA.resetAllCharts = function(state) {
      console.log('Rico trace in resetAllCharts')
    ...
    NETDATA.chartRefresher = function() {
        ...
        var targets = NETDATA.options.targets;
        var len = targets.length;
        console.log('Rico trace in chartRefresher with len=', len)
        var state;
        while(len--) {
            state = targets[len];
            ...
            if(state.library.initialized === false) {
                if(state.library.enabled === true) {
                  ...
                  console.log('chart library "' + state.library_name + '" is not enabled.')
        ...
        if(parallel.length > 0) {
            ...
            $(parallel).each(function() {
              console.log('parallel to autoRefresh')
                this.autoRefresh();
            })
        }
        else {
    ...
    NETDATA.parseDom = function(callback) {
        ...
        while(len--) {
            ...
            console.log('targets looks like: ',targets[len])
            console.log('NETDATA.options.targets looks like: ', NETDATA.chartState(targets[len]))
        }
    ...
    NETDATA.gaugeInitialize = function(callback) {
      console.log('------ come into gaugeInitialize -------')
    ...
    NETDATA.loadRequiredJs = function(index, callback) {
      console.log('============NETDATA.requiredJs[%d] with length=%d', index, NETDATA.requiredJs.length)
      console.dir( NETDATA.requiredJs[index] )
        ...
        if(NETDATA.requiredJs[index].isAlreadyLoaded()) {
          console.log(' come into isAlreadyLoaded()')
            NETDATA.loadedRequiredJs++;
            NETDATA.loadRequiredJs(++index, callback);
            return;
        }

        let currentURL = NETDATA.requiredJs[index].url
        if(NETDATA.options.debug.main_loop === true)
            console.log('xxxxxx   loading ' + currentURL);
        ...
        .done(function() {
          console.log('============NETDATA.requiredJs[%d] url= %s', index, currentURL)
            if(NETDATA.options.debug.main_loop === true)
                console.log('loaded ' + currentURL);
        })
        .fail(function() {
            alert('Cannot load required JS library: ' + currentURL);
        })
        .always(function() {
            NETDATA.loadedRequiredJs++;
    ...
    NETDATA.alarms = {
        ...
        init: function() {
            ...
            console.log('Rico trace in alarms.init:', NETDATA.alarms.server)
